...
class BugSolutionMoreThanOnce(wx.Frame):
    ...
    def run(self):
        ...
        #write to log file
        title_sheet = ['编号'.decode('gbk'), '类别'.decode('gbk'), '标题'.decode('gbk'), '当前状态'.decode('gbk'), 'BUG创建时间'.decode('gbk'), '提出人'.decode('gbk'),'最后更新时间'.decode('gbk'), '问题发现阶段'.decode('gbk'), '是否出现过解决方案被驳回'.decode('gbk'), '驳回人'.decode('gbk'), '提交方案驳回时间'.decode('gbk'), '问题描述'.decode('gbk'), '解决方案'.decode('gbk'), 'Root Cause'.decode('gbk')]
        ...
        sheet.merge_range(0, 0, 0, 13, "%s_BUG统计".decode('gbk') % project_name_selected, formattitle)
        ...
        for index_data, item_data in enumerate(bug_management_sn_list_total):
            ...
            # 是否出现过解决方案被驳回
            sheet.write(2 + index_data, 8, bug_refused_or_not[index_data], formatone)
            # 驳回人
            sheet.write(2 + index_data, 9, bug_username_operation_list[index_data], formatone)
            # 驳回时间
            if bug_operation_time_list[index_data] == "None":
                sheet.write(2 + index_data, 10, bug_operation_time_list[index_data], formatone)
            else:
                sheet.write_datetime(2 + index_data, 10,
            ...
            # 问题描述
            sheet.write(2 + index_data, 11, bug_description_list_total[index_data], formatone)
            # 解决方案
            sheet.write(2 + index_data, 12, bug_solution_list_total[index_data], formatone)
            # root cause
            sheet.write(2 + index_data, 13, bug_rootcause_list_total[index_data], formatone)
